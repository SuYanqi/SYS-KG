<template>
    <el-table :data="relevantBugReports" 
        ...
        style="width: 100%; color: black;" ref="multipleTable" @row-click="rowClick">
        
        <el-table-column type="expand">
        <template #default="props" >
            <div>
                <BugReportCard :bugReport="props.row" />
                <!-- <p v-else> No Data </p> -->
            </div>
        </template>
        </el-table-column>
        ...
        <el-table-column label="Product::Component" min-width="30">
            <template v-slot="props">          
                <div>{{props.row.product}}::{{props.row.component}}</div>
            </template>	
        </el-table-column>
        <el-table-column label="Summary" prop="Summary" min-width="60"/>
        <el-table-column label="Score" prop="Score" min-width="10"/>
    </el-table>    
</template>
<script>
